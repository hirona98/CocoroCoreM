...
CocoroCoreMにおける統合チャット機能のAPI仕様を定義します。5つのチャット機能（テキストチャット、画像付きチャット、通知機能、デスクトップウォッチ機能、リマインダー機能）を統一的に処理し、MemOSとの完全統合を実現します。

## 設計原則

1. **統一API**: 5つの機能を1つのWebSocketエンドポイントで処理
...
**特徴**: 画像データ受信とLLMによる画像分析、コンテキスト統合
...
7. **ストリーミング**: SSE形式からWebSocket形式に変換、テキストバッファリング処理
8. **履歴更新**: 応答完了後に会話履歴の即時更新（非同期）
9. **リマインダー登録**: リマインダータグ検出時は自動登録
10. **終了**: "end"メッセージで完了通知、早期終了によるレスポンス高速化

### テキストバッファリング

- **目的**: ストリーミングの効率化とユーザー体験の向上
- **動作**: 80文字以上のバッファで句読点位置を探し、適切なタイミングで送信
- **タイムアウト**: 2秒以内に送信条件を満たさない場合は強制送信
- **記憶参照除去**: 送信前にMemOSの記憶参照タグを除去
