...
from memos.templates.mem_reader_prompts import SIMPLE_STRUCT_MEM_READER_EXAMPLE
...
class CocoroMOSProduct(MOSProduct):
    ...
    class CocoroMemReader(SimpleStructMemReader):
        ...
        def _process_chat_data(self, scene_data_info: List[str], info: dict[str, Any]) -> List[TextualMemoryItem]:
            ...
            """
            # 日本語プロンプトを使用
            prompt = CocoroMOSProduct.SIMPLE_STRUCT_MEM_READER_PROMPT_JP.replace(
                "${conversation}", "\n".join(scene_data_info)
            )
            if self.config.remove_prompt_example:
                # 日本語の例文部分を削除
                prompt = prompt.replace(CocoroMOSProduct.SIMPLE_STRUCT_MEM_READER_EXAMPLE_JP, "")
        ...
        def _process_doc_data(self, scene_data_info: dict, info: dict[str, Any]) -> List[TextualMemoryItem]:
            ...
            messages = [
                [
                    {
                        "role": "user",
                        "content": CocoroMOSProduct.SIMPLE_STRUCT_DOC_READER_PROMPT_JP.replace("{chunk_text}", chunk.text),
                    }
                ]
                for chunk in chunks
            ]
    ...
    def query_rewrite(self, query: str, dialogue: str) -> dict:
        ...
        """
        prompt = self.QUERY_REWRITING_PROMPT_JP.format(
            dialogue=dialogue,
            query=query
        )
    ...
    def cot_decompose(self, query: str) -> dict:
        ...
        """
        prompt = self.COT_DECOMPOSE_PROMPT_JP.format(query=query)
    ...
    def synthesis_response(self, qa_text: str) -> str:
        ...
        Returns:
            str: 合成された回答
        """
        prompt = self.SYNTHESIS_PROMPT_JP.format(qa_text=qa_text)
    ...
    def __init__(self, default_config=None, max_user_instances=1, system_prompt_provider: Optional[Callable[[], Optional[str]]] = None):
        ...
        if hasattr(self, 'config') and hasattr(self.config, 'mem_reader'):
            self.mem_reader = self.CocoroMemReader(self.config.mem_reader.config)
