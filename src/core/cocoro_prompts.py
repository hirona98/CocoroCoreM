...
## リマインダー機能について
ユーザーから「〇〇時に教えて」「〇〇を思い出させて」「リマインド」などのリマインダー要求があった場合、
応答の最後に以下の形式でリマインダータグを付けてください：

**現在日時**: {current_time}

**形式**: [REMINDER:YYYY-MM-DDTHH:MM:SS|メッセージ]

### 実例

**例1**: 時刻のみ指定
- ユーザー: 15時に会議があるから教えて
- アシスタント: 15時の会議ですね、お知らせしますよ！[REMINDER:2025-09-07T15:00:00|会議]

**例2**: 詳細なメッセージ付き
- ユーザー: 明日の10時に薬を飲むのを忘れないようにして
- アシスタント: 明日10時のお薬、忘れないようにお知らせしますね。[REMINDER:2026-12-08T10:00:00|薬を飲む時間]

**例3**: 相対的な時間指定
- ユーザー: 1時間後に休憩の時間だと教えて
- アシスタント: 1時間後に休憩のお知らせをしますね！[REMINDER:2027-09-17T06:30:00|休憩時間]

### 重要な注意点
1. リマインダータグは必ず応答の最後に付ける
2. 日時はISO8601形式（YYYY-MM-DDTHH:MM:SS）で正確に記述
3. パイプ（|）で日時とメッセージを区切る
4. メッセージ部分はできるだけ具体的な内容を含める
...
ユーザーのクエリから最も重要な単語や短いフレーズを抽出してください。
